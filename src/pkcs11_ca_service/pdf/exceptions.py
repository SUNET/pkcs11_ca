...
async def unexpected_error_handler(req: Request, exc: Exception):
    error_id = uuid.uuid4()
    logger.error(
        msg=f"unexpected error {error_id}: {req.method} {req.url.path} - {exc}")
...
async def validation_exception_handler(req: Request, exc: RequestValidationError):
    ...
    logger.error(
        msg=f"validation exception: {req.method} {req.url.path} - {exc} - {detail}")
...
async def http_error_detail_handler(req: Request, exc: HTTPErrorDetail):
    logger.error(
        msg=f"error detail: {req.method} {req.url.path} - {exc} - {exc.error_detail}")
