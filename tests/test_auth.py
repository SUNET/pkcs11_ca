...
class TestAuth(unittest.TestCase):
    ...
    def test_auth_nonce(self) -> None:
        ...
        # No nonce
        jwt_headers = {"url": "http://localhost:8005/search/ca"}
    ...
    def test_auth_url(self) -> None:
        ...
        # No url in token
        req = requests.head("http://localhost:8005/new_nonce", timeout=5)
        ...
        nonce = req.headers["Replay-Nonce"]
        self.assertTrue(req.status_code == 200)
        jwt_headers = {"nonce": nonce, "url": "http://localhost:8005/search/ca_wrong_url"}
        jwk_key_data = pem_key_to_jwk(pub_key1.decode("utf-8"))
        encoded = jwt.encode(jwk_key_data, priv_key1.decode("utf-8"), algorithm="PS256", headers=jwt_headers)
        request_headers = {"Authorization": "Bearer " + encoded}
        req = requests.get("http://localhost:8005/search/ca", headers=request_headers, timeout=5)
    ...
    def test_auth(self) -> None:
        ...
        req = requests.head("http://localhost:8005/new_nonce", timeout=5)
        nonce = req.headers["Replay-Nonce"]
        jwt_headers = {"nonce": nonce, "url": "http://localhost:8005/search/ca"}
        jwk_key_data = pem_key_to_jwk(pub_key1.decode("utf-8"))
        ...
        req = requests.get("http://localhost:8005/new_nonce", timeout=5)
        nonce = req.headers["Replay-Nonce"]
        self.assertTrue(req.status_code == 200)
        jwt_headers = {"nonce": nonce, "url": "http://localhost:8005/search/ca"}
        jwk_key_data = pem_key_to_jwk(pub_key1.decode("utf-8"))
        ...
        # Test lib auth
        request_headers = {
            "Authorization": create_jwt_header_str(pub_key1, priv_key1, "http://localhost:8005/search/ca")
        }
        req = requests.get("http://localhost:8005/search/ca", headers=request_headers, timeout=5)
        self.assertTrue(req.status_code == 200)
    ...
    def test_aia_cdp_auth(self) -> None:
        ...
        """
        req = requests.get("http://localhost:8005/ca/acac22352343423", timeout=5)
        self.assertTrue(req.status_code == 404)
        req = requests.get("http://localhost:8005/crl/acac22352343423", timeout=5)
        self.assertTrue(req.status_code == 404)
    ...
    def test_auth_secp256r1(self) -> None:
        ...
        nonce = req.headers["Replay-Nonce"]
        self.assertTrue(req.status_code == 200)
        jwt_headers = {"nonce": nonce, "url": "http://localhost:8005/search/public_key"}
