...
class TestCrl(unittest.TestCase):
    ...
    def test_crl(self) -> None:
        ...
        # create a crl
        request_headers = {"Authorization": create_jwt_header_str(pub_key, priv_key, "https://localhost:8005/crl")}

        data = json.loads('{"ca_pem": ' + '"' + cas[0].replace("\n", "\\n") + '"' + "}")
        req = requests.post("https://localhost:8005/crl", headers=request_headers, json=data, timeout=5, verify=False)
        ...
        # get the crl
        request_headers = {
            "Authorization": create_jwt_header_str(pub_key, priv_key, "https://localhost:8005/search/crl")
        }

        data = json.loads('{"pem": ' + '"' + crl1.replace("\n", "\\n") + '"' + "}")
        req = requests.post(
            "https://localhost:8005/search/crl", headers=request_headers, json=data, timeout=5, verify=False
        )
        self.assertTrue(req.status_code == 200)
